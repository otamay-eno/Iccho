...
import { v4 as uuidv4 } from 'uuid'; // Actually Supabase can generate IDs, but keeping uuid for client side opt? Server action can handle ID.
// Wait, Server Action should handle ID generation usually, or let Supabase do it (default uuid).
// I will not send ID from client.
...
const AddTransaction = ({ members, onComplete }) => {
    ...
    const [formData, setFormData] = useState({
        title: '',
        amount: '',
        payer_id: members.length > 0 ? members[0].id : '',
        for_who_ids: members.map(m => m.id), // Default to everyone
    ...
    const handlePayerChange = (id) => {
        setFormData(prev => ({ ...prev, payer_id: id }));
    };

    const handleTargetChange = (id) => {
        setFormData(prev => {
            const current = prev.for_who_ids;
            const newTargets = current.includes(id)
                ? current.filter(n => n !== id)
                : [...current, id];
            return { ...prev, for_who_ids: newTargets };
        });
    };

    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.amount || !formData.title || formData.for_who_ids.length === 0) {
            alert("必須項目を入力してください");
            return;
        }
        ...
        // ID is auto-generated by Supabase usually. But user code used uuidv4.
        // I will let Supabase generate ID or generate it in Server Action. 
        // I'll send data without ID.
        const payload = {
            ...formData
        };
        ...
        try {
            ...
            setFormData({
                ...
                amount: '',
                payer_id: members.length > 0 ? members[0].id : '',
                for_who_ids: members.map(m => m.id),
                date: new Date().toISOString().split('T')[0]
            });
            ...
            >
                ...
                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ mt: 1 }}>
                    {members.map(m => (
                        <Chip
                            key={m.id}
                            avatar={<Avatar sx={{ bgcolor: formData.payer_id === m.id ? 'primary.dark' : 'grey.300' }}>{m.name[0]}</Avatar>}
                            label={m.name}
                            onClick={() => handlePayerChange(m.id)}
                            color={formData.payer_id === m.id ? "primary" : "default"}
                            variant={formData.payer_id === m.id ? "filled" : "outlined"}
                            clickable
                            sx={{
                                height: 40,
                                borderRadius: 20,
                                borderWidth: formData.payer_id === m.id ? 0 : 1,
                                fontSize: '1rem',
                                p: 0.5,
                                borderColor: '#cfd8dc'
                            }}
